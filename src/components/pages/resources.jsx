...
const Card = ({ children, className = "", onClick, clickable = false }) => (
  <div 
    className={`bg-slate-800 border border-slate-700 rounded-lg ${clickable ? 'cursor-pointer hover:border-indigo-500 transition-colors' : ''} ${className}`}
    onClick={onClick}
  >
    {children}
  </div>
...
const CardTitle = ({ children, className = "" }) => <h3 className={`text-xl font-mono text-white ${className}`}>{children}</h3>;
    ...
    >
      ...
      {type === 'youtube' && (
        <div className="flex items-center justify-between">
          ...
          <span className="text-indigo-300 text-sm">{resource.views} views</span>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="bg-slate-900 min-h-screen py-12 px-4">
      <div className="max-w-4xl mx-auto text-center mb-12">
        <h2 className="text-3xl md:text-4xl font-mono text-white mb-4">Learning Resources</h2>
        <p className="text-gray-400 text-lg">Get personalized learning resources for your career path</p>
      </div>

      <form onSubmit={fetchResources} className="max-w-3xl mx-auto space-y-6 mb-8 bg-slate-800 p-6 rounded-lg">
        <div>
          ...
          <Card>
            ...
            <CardContent className="flex flex-wrap gap-2">
              {resources.keySkills.map((skill, index) => (
                <span key={index} className="bg-indigo-600/30 text-indigo-400 px-3 py-1 rounded-full text-sm">
                  {skill}
                </span>
              ))}
            </CardContent>
